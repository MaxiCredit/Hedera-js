...
const fixer = {
    method: 'GET',
    uri: 'http://data.fixer.io/api/latest?access_key=...'
};

const dragonglassRequestEvents = {
	method: 'GET',
    uri: 'https://api-testnet.dragonglass.me/hedera/api/contracts/0.0.17147/calls?contractMethodName=519acb87&consensusStartInEpoch=159578574400',
    //uri: 'https://api-testnet.dragonglass.me/hedera/api/contracts/0.0.73154/callCount',
...
const ethPrKey = Buffer.from(process.env.ETH_PR_KEY, "hex");


var express = require('express');
var router = express.Router();
const MXContractId = "0.0.17137";
const TestUsers = "0.0.17114";
const TestCredit = "0.0.17184";
const MXx = "0.0.17147";
...
router.post('/sendHbar', (req, res) => {
    ...
    req.on('end', () => {
        ...
        if(client[1] != "ERROR" && address && amount > 0) {
            ...
            lib.hederaAccountInfo(lib.createHederaClient(accountID, privateKey), address).catch(error => {
                console.log("ERROR when send Hbar (client check)" + error);
                response = "ERROR when send Hbar " + error.status + ":" + error.transactionId;
                res.send(response);
            }).then(result => {
                //console.log(result.isDeleted);
                if(result) {
                    let hbarTX = new CryptoTransferTransaction()
                        .addSender(accountID, sum)
                        .addRecipient(result.accountId, sum)
                        .build(client);
                    hbarTX.execute(client).catch((e) => {
                        console.log("ERROR when send Hbar" + e);
                        response = "ERROR when send Hbar" + e;
                    }).then((r) => {
                        console.log("Send Hbar", r);
                        response = "SUCCESS";
                        res.send(response);
                    });
                }
...
router.post('/sendMaxit', (req, res) => {
    ...
    req.on('end', () => {
        ...
        let response = "";
        if(client[1] != "ERROR" && address && amount > 0) {
            lib.hederaAccountInfo(lib.createHederaClient(mxOwner, ownerPrKey), address).catch(e => {
                console.log("ERROR when send MX " + e);
                response = "ERROR when send MX " + e.status + ":" +  e.transactionId;
                ...
                if(result) {
                    ...
                    lib.sendContract(MXContractId, "transfer", params, client).catch(error => {
                        console.log("ERROR when send MX: " + error);
                        response = "ERROR when send MX " + e.status + ":" +  e.transactionId;
                        res.send(JSON.stringify(response));
                    }).then((result) => {
    ...
    req.on('end', () => {
        ...
        if(client[1] != "ERROR" && firstname && lastname && birth && motherfirst && motherlast && id && citizen && country && city && address && email && phone && birthname) {
            ...
            lib.sendContract(TestUsers, "registerUser", params, client).catch(error => {
                ...
                console.log(result.receipt.status);
                res.send("SUCCESS");
                lib.transporter.sendMail({
                    from: "info@maxicreditcompany.com", // sender address
                    to: "mihaly.e.v@gmail.com", // list of receivers
                    subject: "MaxiCredit új regisztráció", // Subject line
                    html: "<b>Üdvözlünk</b><br><br><p>Név " + firstname + " " + lastname + "</p><p>Számla szám: " + accountID + 
                    "</p><p>Email: " + email + "</p><br><br><p>MaxiCredit</p>", // html body
                  }, function(err, info){
                    if (err) {
                      console.log("Reg email: " + err);
                    } else {
...
router.post('/modifyUser', (req, res) => {
    ...
    req.on('end', () => {
        ...
        let firstname = clientData[2].slice(clientData[2].indexOf('=') + 1);
        let lastname = clientData[3].slice(clientData[3].indexOf('=') + 1);
        let id = clientData[4].slice(clientData[4].indexOf('=') + 1);
        let citizen = clientData[5].slice(clientData[5].indexOf('=') + 1);
        ...
        if(client[1] != "ERROR") {
            let param = new ContractFunctionParams()
                .addAddress(lib.solidityAddress(parseInt(lib.accountNumber(accountID))));
            lib.callContract(TestUsers, "getUserByAddress", param, client).catch(e => {
                res.send("ERROR when getID at modify user: " + e);
            }).then(r => {
                console.log("Modify user ID: ", r.getUint256(0).toString());
                let phoneNumber = new BigNumber(parseInt(phone));
                let params = new ContractFunctionParams()
                    .addUint256(r.getUint256(0))
                    .addString(firstname)
                    .addString(lastname)
                    .addString(id)
                    .addString(citizen)
                    ...
                    .addString(email)
                    .addUint256(phoneNumber);
                lib.sendContract(TestUsers, "modify", params, client).catch(error => {
                    res.send("ERROR at modify user: " + error);
                }).then((result) => {
                    console.log(result.receipt.status);
                    res.send("SUCCESS");
                }); 
            ...
            res.send("ERROR when Modify user, wrong or missing argument");
        }
    });
});

router.post('/setAddressToAdd', (req, res) => {
    const decoder = new StringDecoder('utf-8');
    let buffer = '';
    req.on('data', (data) => {
        buffer += decoder.write(data);
    ...
    req.on('end', () => {
        buffer += decoder.end();
        let clientData = buffer.split('&');
        console.log("ADD address", buffer);
        let privateKey = clientData[0].slice(clientData[0].indexOf('=') + 1);
        let accountID = clientData[1].slice(clientData[1].indexOf('=') + 1);
        let address = clientData[2].slice(clientData[2].indexOf('=') + 1);
        ...
        if(client[1] != "ERROR") {
            let params = new ContractFunctionParams()
                .addAddress(lib.solidityAddress(parseInt(lib.accountNumber(address))));
            lib.sendContract(TestUsers, "setAddressToAdd", params, client).catch(error => {
                console.log("ERROR when set address to add: " + error);
                res.send("ERROR when set address to add: " + error);
            }).then((result) => {
                console.log(result.receipt.status);
                res.send("SUCCESS");
            }); 
        } else {
            res.send("ERROR when set address to add, wrong or missing argument");
        }
    });
});
    ...
    req.on('end', () => {
        ...
        if(client[1] != "ERROR") {
            let param = new ContractFunctionParams()
                .addAddress(lib.solidityAddress(parseInt(lib.accountNumber(address))));
            lib.callContract(TestUsers, "getUserByAddress", param, client).catch(e => {
                res.send("ERROR when getID at confirm address: " + e);
            }).then(r => {
                console.log("confirm address user ID: ", r.getUint256(0).toString());
                let params = new ContractFunctionParams()
                    .addUint256(r.getUint256(0));
                lib.sendContract(TestUsers, "addAddressToUser", params, client).catch(error => {
                    res.send("ERROR when confirm address: " + error);
                }).then((result) => {
                    console.log(result.receipt.status);
                    res.send("SUCCESS");
                }); 
            ...
            res.send("ERROR when confirm address, wrong or missing argument");
        }
    });
});
...
router.post('/createCreditOffer', (req, res) => {
    ...
    req.on('end', () => {
        ...
        let addressArray = [lib.solidityAddress("0"), lib.solidityAddress("0")];
...
router.post('/createCreditClaim', (req, res) => {
    ...
    req.on('end', () => {
        ...
        if(client[1] != "ERROR" && allowance > 1999 && amount > 1999 && type && length > 299 && periods > 0 && interest > 0 && last > 0) {
            ...
            .addUint256(new BigNumber(interest))
            .addUint256(new BigNumber(last))
            .addAddress(lib.solidityAddress("0"))
            .addUint256(new BigNumber(0));
            lib.sendContract(MXContractId, "approve", args, client).catch(er => {
...
router.post('/acceptOfferShop', (req, res) => {
    const decoder = new StringDecoder('utf-8');
    let buffer = '';
    req.on('data', (data) => {
        buffer += decoder.write(data);
    ...
    req.on('end', () => {
        ...
        let offerID = clientData[3].slice(clientData[3].indexOf('=') + 1);
        let amount = clientData[4].slice(clientData[4].indexOf('=') + 1);
        let shop = clientData[5].slice(clientData[5].indexOf('=') + 1);
        ...
        if(client[1] != "ERROR" && allowance > 1999 && offerID && amount > 1999) {
            ...
            .addUint256(new BigNumber(offerID))
            .addUint256(new BigNumber(amount))
            .addAddress(lib.solidityAddress(parseInt(shop)));
            console.log("PARAMS: " + offerID + " " + amount + " " + allowance);
            lib.sendContract(MXContractId, "approve", args, client).catch(er => {
                res.send("ERROR when aprove for accept offer: " + er);
            }).then((re) => {
                console.log("ALLOWANCE");
                console.log(re.receipt.status);
                lib.sendContract(TestCredit, "acceptCreditOfferShop", params, client).catch((e) => {
                    console.log(e.record);
                    res.send("ERROR when accept offer: " + e);
                }).then((result) => {
                    console.log("ACCEPT CREDIT OFFER: " + result.receipt.status);
...
router.post('/sellLoan', (req, res) => {
    ...
    req.on('end', () => {
        ...
        let client = lib.createHederaClient(accountID, privateKey);
        if(client[1] != "ERROR" && loanAddress && price > 0) {
            let loanAddressStr = "0.0." + parseInt(loanAddress, 16);
            console.log("Loan to sale: " + loanAddressStr);

            let params = new ContractFunctionParams()
            .addUint256(new BigNumber(price));
            lib.sendContract(loanAddressStr, "setToSale", params, client).catch(error => {
                res.send("ERROR when sell loan: " + error);
            }).then((result) => {
                console.log("SELL: " + result.receipt.status);
                res.send("SUCCESS");
            }); 
        } else {
            res.send("ERROR when restruct loan, wrong or missing argument");
...
router.post('/getUserId', (req, res) => {
    ...
    req.on('end', () => {
        ...
        if(client[1] != "ERROR") {
            let params = new ContractFunctionParams()
                .addAddress(lib.solidityAddress(parseInt(lib.accountNumber(user))));
            lib.callContract(TestUsers, "checkUsersAddress", params, client).catch(e => {
                console.log("ERROR at checkUserByAddress: " + e);
                res.send("ERROR at checkUserByAddress: " + e);    
            }).then(r => {
                console.log("Check user: " + user + " " + r.getBool(0));
                if(r.getBool(0)) {
                    lib.callContract(TestUsers, "getUserByAddress", params, client).catch((error) => {
                        console.log("ERROR at getUserByAddress: " + error);
                        res.send("ERROR at getUserByAddress: " + error);
                    }).then((result) => {
                        res.send(result.getUint256(0).toString());
                    });
                } else {
                    res.send("Not user: " + user);
                }
            });
        }   
    });
...
router.post('/getUserDetails', (req, res) => {
    ...
    req.on('end', () => {
        ...
        if(client[1] != "ERROR" && userId) {
            let param = new ContractFunctionParams()
                .addAddress(lib.solidityAddress(parseInt(lib.accountNumber(user))));
            lib.callContract(TestUsers, "checkUsersAddress", param, client).catch(e => {
                console.log("ERROR at checkUserByAddress: " + e);
                res.send("ERROR at checkUserByAddress: " + e);    
            }).then(r => {
                console.log("Check user: " + user + " " + r.getBool(0));
                if(r.getBool(0)) {
                    ...
                    lib.callContract(TestUsers, "getUserPrivateData1", params, client).catch((error) => {
                        ...
                        lib.callContract(TestUsers, "getUserPrivateData2", params, client).catch((err) => {
                            ...
                            lib.callContract(TestUsers, "getUserPublicData", params, client).catch((erro) => {
                                console.log("ERROR at getUserPublicData: " + erro);
                                res.send("ERROR at getUserPublicData: " + erro);
                            }).then((resu) => {
                                ...
                                lib.callContract(TestUsers, "getPendingUser", params, client).catch((er) => {
                                    console.log("ERROR at getPendingUser: " + er);
                                    res.send("ERROR at getPendingUser: " + er);
                                }).then((re) => {
...
router.post('/getUserIdAdmin', (req, res) => {
    const decoder = new StringDecoder('utf-8');
    let buffer = '';
    req.on('data', (data) => {
        buffer += decoder.write(data);
    ...
    req.on('end', () => {
        buffer += decoder.end();
        clientData = buffer.split('&');
        let user = clientData[0].slice(clientData[0].indexOf('=') + 1);
...
router.post('/sellMXtoTether', (req, res) => {
    const decoder = new StringDecoder('utf-8');
    let buffer = '';
    req.on('data', (data) => {
        buffer += decoder.write(data);
    ...
    req.on('end', () => {
        ...
        let accountID = clientData[1].slice(clientData[1].indexOf('=') + 1);
        let amount = clientData[2].slice(clientData[2].indexOf('=') + 1);
        let ethAddrSeller = clientData[3].slice(clientData[3].indexOf('=') + 1);
        console.log("ETH:", ethAddrSeller, typeof(ethAddr), amount, typeof(amount));
    
        const client = lib.createHederaClient(accountID, privateKey);
        if(client[1] != "ERROR" && amount > 0) {
            let param = new ContractFunctionParams()
                .addAddress(lib.solidityAddress(parseInt(lib.accountNumber(MXx))))
                .addUint256(new BigNumber(parseInt(amount)));
            lib.sendContract(MXContractId, "approve", param, client).catch(e => {
                console.log("Approve for sell MX to Tether ERROR:", e.record.receipt)
                res.send("ERROR when approve for sell MX to Tether:" + e.transactionId);
            }).then(r => {
                console.log("APPROVE sell MX to Tether:", r.receipt.status);
                let params = new ContractFunctionParams()
                    .addUint256(new BigNumber(parseInt(amount)))
                    .addString(ethAddrSeller);
                //convertFromHederaMX(uint _amount, string memory _ethAddress)
                lib.sendContract(MXx, "convertFromHederaMX", params, client).catch(error => {
                    console.log("Sell MX to Tether ERROR:", error.record.receipt);
                    res.send("ERROR when sell MX to Tether:" + error.transactionId);
                }).then(result => {
                    console.log("SELL MX:", result.receipt.status, result.transactionId);
                    let hederaTxId = result.transactionId.accountId.shard.toString() + result.transactionId.accountId.realm.toString() + result.transactionId.accountId.account.toString() + result.transactionId.validStart.seconds.toString() + result.transactionId.validStart.nanos.toString();
                    let currentEvent = [ethAddrSeller, parseInt(amount), hederaTxId];
                    let events = [];
                    events.push(currentEvent);
                    //console.log("SELL MX:", events);
                    web3.eth.getTransactionCount(ethAddr, (error, result) => {
                        if(!error) {
                            payMXSellers(events, 1, result);
                        } else {
                            console.log("GET NONCE ERROR", error);
                        }
                    });
                    /*
                    fs.writeFile("sellMX.json", JSON.stringify(result), (err) => {
                        if(err) console.log("fs error");
                    });*/
                    res.send("SUCCESS");
                });
            });
...
router.get('/paypalCheck', (req, res) => {
    ...
    paypalClient.execute(request).catch(e => console.log(e)).then(r => {
        let paidAmount = parseInt(r.result.purchase_units[0].payments.captures[0].seller_receivable_breakdown.net_amount.value);
        let paidCurrency = r.result.purchase_units[0].payments.captures[0].seller_receivable_breakdown.net_amount.currency_code;
        console.log(r.result.status, paidAmount, paidCurrency);  
        console.log("MXHUF:", MXHUF);
        console.log("toPay:", mxAmount * usdPrice / 10000);
        let paidProperlyCheck = false;
        switch(paidCurrency) {
            case 'USD' :
                if(parseInt(mxAmount * usdPrice / 10000) <= paidAmount) {
                    paidProperlyCheck = true;
                } else {
                    paidProperlyCheck = false;
                }
                break;
            case 'HUF' :                 
                if(parseInt(parseFloat(mxAmount) * parseFloat(MXHUF) - 10) <= paidAmount) {
                    paidProperlyCheck = true;
                } else {
                    paidProperlyCheck = false;
                }
                break;
            default : paidProperlyCheck = false;
        }
        //convertToHederaMX(address _buyer, uint _amount)
        if(paidProperlyCheck) {
            const hederaClient = lib.createHederaClient(6105, "...");
            ...
            res.send("ERROR, paid amount mismatch!");
        }
    });
});

router.get('/buyStarterPack', (req, res) => {
    console.log("Paypal:", req.query);
    let orderId = req.query.orderId;
    let hederaAccount = req.query.account;
    //let mxAmount = req.query.mxAmount;
    console.log(orderId, hederaAccount);
    let request = new paypal.orders.OrdersCaptureRequest(orderId);
    request.requestBody({});
    paypalClient.execute(request).catch(e => console.log(e)).then(r => {
        let paidAmount = parseInt(r.result.purchase_units[0].payments.captures[0].seller_receivable_breakdown.net_amount.value);
        ...
        switch(paidCurrency) {
            case 'USD' :
                if(paidAmount >= 5000 * usdPrice / 10000) {
                    paidProperlyCheck = true;
                } else {
                    paidProperlyCheck = false;
                }
        ...
        console.log(r.result.status, paidAmount);  
        //check users account (hederaAccount)
        if(paidProperlyCheck) {
            const hederaClient = lib.createHederaClient(6105, "...");
            if(hederaClient[1] != "ERROR") {
                let params = new ContractFunctionParams()
                    .addAddress(lib.solidityAddress(parseInt(lib.accountNumber(hederaAccount))))
                    .addUint256(new BigNumber(4000));
                lib.sendContract(MXx, "convertToHederaMX", params, hederaClient).catch(error => {
                    console.log("ERROR when ConvertToHederaMX" + error);
                    res.send("ERROR when ConvertToHederaMX" + error);
                }).then(result => {
                    console.log("ConvertToHederaMX", result.receipt.status);
                    let hbarTX = new CryptoTransferTransaction()
                        .addSender(lib.solidityAddress(6105), new Hbar("200"))
                        .addRecipient(lib.solidityAddress(parseInt(lib.accountNumber(hederaAccount))), new Hbar("200"))
                        .build(hederaClient);
                    hbarTX.execute(hederaClient).catch((e) => {
                        console.log("ERROR when send Hbar" + e);
                        response = "ERROR when send Hbar" + e;
                    }).then((r) => {
                        console.log("Send Hbar", r);
                        response = "SUCCESS, you received 4000 MX and 200Hbars";
                        res.send(response);
                    });
                });
            } else {
...
router.get('/mxhuf', (req, res) => {
    console.log("mxhuf:", MXHUF);
    res.send(MXHUF.toString());
});
...
let mxxEvents = () => {
    rp(dragonglassRequestEvents).catch(e => console.log("ERROR at get xchange events")).then((r) => {
        let events = [];
        if(r) {
            //console.log(r);
            ...
            if(r.data.length > lastMxxEventSize) {
                ...
                web3.eth.getTransactionCount(ethAddr, (error, result) => {
                    if(!error) {
                        payMXSellers(events, newEventCount, result);
                    } else {
                        console.log("GET NONCE ERROR", error);
                    }
                });
...
var payMXSellers = (events, counter, prevNonce) => {
    if(events[counter - 1]) {
        changeFromETH.methods.convertToBytes32(events[counter - 1][2]).call({from : ethAddr}).catch(console.log).then((resu) => {
            ...
            changeFromETH.methods.ethTXPayStatus(resu).call({from : ethAddr}).catch(console.log).then((re) => {
                ...
                if(!re) {
                    ...
                    ethTX.setTx(100000000000, 3000000, '0x17070EdD831C8BE6CC6f49A429d37134D7Dfb745', functionAbi, 3, ethPrKey, prevNonce, (err, res) => {
                        if(!err) {
                            counter--;
                            lastMxxEventSize++;
                            ...
                            if(counter > 0) {
                                prevNonce++;
                                payMXSellers(events, counter, prevNonce);
                            }
                        } else {
                            console.log("PAY seller ERROR:", err);
                        }
                    });
                } else {
                    counter--;
...
let mxxBalance = () => {
    let client = lib.createHederaClient(mxOwner, ownerPrKey);
    lib.callContractNoArgs(MXx, "getBalances", client).catch(console.log).then((r) => {
        console.log("MXX balance: " + r.getUint256(0).toString() + "MX " + parseInt(r.getUint256(1)) / 100000000 + "Hbar");
        changeFromETH.methods.getBalances().call().catch(console.log).then(re => {
           console.log("MXX balance: " + re[0] / 1000000 + "MCT " + re[1] + "wei"); 
        });
    });
}

setInterval(mxxBalance, 300000);

//lib.hederaAccountInfo(lib.createHederaClient(mxOwner, ownerPrKey), "0.0.24011").catch(e => console.log(e.status)).then(r => console.log(r.isDeleted));

let fiatPrices = () => {
    rp(fixer).catch().then(result => {
        let fixer = JSON.parse(result);
        console.log("USDHUF:", Math.floor(fixer.rates.HUF / fixer.rates.USD), "MXUSD:", usdPrice);
        MXHUF = Math.floor(fixer.rates.HUF / fixer.rates.USD) / usdPrice;
    });
}

//fiatPrices();
setInterval(fiatPrices, 1080000);
