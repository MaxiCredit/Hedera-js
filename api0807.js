...
function createModal(type, args) {
    ...
    modalHeader.style.borderBottom = "1px black solid";
    modalHeader.style.padding = "5px";
    let modalBody = document.createElement("div");
    modalBody.setAttribute("class", "modal-body");
    modalBody.setAttribute("id", "modal-body");
    modalBody.style.borderBottom = "1px black solid";
...
function setModal(type, args, modalHeader, modalBody) {
    ...
    switch(type) {
        ...
        case "saveNewAccount" : saveNewAccountModal(modalHeader, modalBody);
            break;
        case "acceptOffer" : acceptOfferModal(modalHeader, modalBody, args);
            break;
        case "register" : registerModal(modalHeader, modalBody, args);
            break;
        case "pay" : payModal(modalHeader, modalBody, args);
            break;
        case "buyMX" : buyMXModal(modalHeader, modalBody, args);
            break;
        case "listOffers" : listOfferModal(modalHeader, modalBody, args);
            break;
        default :
    }
}
...
function pay(url, sum) {
    var payButton = document.createElement("button");
    payButton.innerHTML = "Fizetés Maxittal";
    var amount = parseInt(document.getElementById(sum).value);
    console.log("AMOUNT:", amount, document.getElementById(sum));
...
function buyMXModal(modalHeader, modalBody, args) {
    var buyMXTitle = document.createElement("h3");
    buyMXTitle.innerHTML = "Maxit vásárlás";

    var mxToBuyAlert = document.createElement("p");
    mxToBuyAlert.innerHTML = "<b>" + args[0] + "</b>";

    var mxToBuyMin = document.createElement("p");
    mxToBuyMin.innerHTML = "A vásárlás befejezéséhez vegyél még " + args[1] + "Maxitot, vagy vásárolj hitel igénybevételével";

    var mxToBuyText = document.createElement("p");
    mxToBuyText.innerHTML = "Vásárlandó mennyiség";
    var mxToBuyAmount = document.createElement("input");
    mxToBuyAmount.setAttribute("id", "paypal-mx-amount");

    var mxToBuyButton = document.createElement("div");
    mxToBuyButton.setAttribute("id", "paypal");
    mxToBuyButton.style.marginTop = "10px";
    //mxToBuyButton.innerHTML = "Vásárlás";

    modalHeader.appendChild(buyMXTitle);
    modalBody.appendChild(mxToBuyAlert);
    modalBody.appendChild(mxToBuyMin);
    modalBody.appendChild(mxToBuyText);
    modalBody.appendChild(mxToBuyAmount);
    modalBody.appendChild(mxToBuyButton);

    function loadPaypal() {
        paypal(args[2]);
    }
    setTimeout(loadPaypal, 10);
}

function paypal(minAmount) {
    paypal_sdk.Buttons({
        createOrder: function(data, actions) {
          // This function sets up the details of the transaction, including the amount and line item details.
          mxAmount = parseInt(document.getElementById("paypal-mx-amount").value);
          
          //toPayUSD = parseFloat((mxAmount * 10 + 300) / 966).toFixed(2);
          toPayHUF = parseInt((mxAmount * 3100 + 90000) / 966);
          console.log("Paypal MX vásárlás: " + mxAmount + " " + account + " " + toPayHUF);
        ...
        onApprove: function(data, actions) {
            ...
            return actions.order.capture().then(function(details) {
              ...
              myAjax("GET", "http://" + host + "/paypalCheck?orderId=" + details.id + "&account=" + account + "&mxAmount=" + mxAmount, "", function(data, status) {
                console.log(data);
                console.log(status);
                if(status == mxAmount + " MX has been sent to " + account) {
                    alert(mxAmount + " Maxit jóváírásra került a számládon, klikkelj a Fizetés gombra!");
                    closeModal();
                    createModal("pay", minAmount);
                }
              });
            });
        }
...
function buyStarterPack() {
    closeModal();
    var starterPackArray = ["Elégtelen egyenleg!", 0, 2000, 100];
    createModal("buyMX", starterPackArray);    
}

function loginAjax(req, type, amount) {
    myAjax("POST", "http://" + host + "/client", req, function(err, res) {
        if(!err) {
            //console.log(res, type, typeof(type));
            //console.log("LOGGED");
            var data = JSON.parse(res);
            if(data.hbarBalance > 10) {
                if(type == "pay") {
                    if(data.mxBalance < amount) {
                        console.log("Insufficent balance");
                        var balanceArray = ["Elégtelen egyenleg!", amount - data.mxBalance, amount, 0];
                        closeModal();
                        createModal("buyMX", balanceArray);
                    } else {
                        console.log("pay");
                        var sendMXReq = "pr=" + prKey + "&acc=" + account + "&to=" + shopHederaAddress + "&amount=" + amount;
                        myAjax("POST", "http://" + host + "/sendMaxit", sendMXReq, function(error, result) {
                            if(!error) {
                                console.log(result);
                ...
                if(type == "loan") {
                    ...
                    if(score > 0) {
                        ...
                        closeModal();
                        createModal("register", amount);
                    }
                }      
            } else {
                alert("Alacsony Hbar egyenleg, nem indítható művelet!");
            }
...
function newAccount() {
    myAjax("POST", "http://" + host + "/createAccount", "", function(err, res) {
        if(!err) {
...
function loginModal(modalHeader, modalBody, args) {
    ...
    if(account == "" && prKey == "") {
        ...
        loginKey.innerHTML = "<button onclick=document.getElementById('login-key').click()>Tallózás</button><input type=file id=login-key style=display:none onchange=setLoginText()>";
        ...
        let starterPackButton = document.createElement("button");
        starterPackButton.setAttribute("class", "btn btn-primary");
        starterPackButton.setAttribute("onclick", "buyStarterPack()");
        let starterPackButtonText = document.createTextNode("Kezdő csomag");
        starterPackButton.appendChild(starterPackButtonText);
        loginDiv.appendChild(starterPackButton);
...
function listOfferModal(modalHeader, modalBody, amount) {
    var listOfferHead = document.createElement("h3");
    listOfferHead.innerHTML = "Ajánlatok";
    modalHeader.appendChild(listOfferHead);

    var amountToBorrow = document.createElement("p");
    amountToBorrow.innerHTML = "Ekkora összegű hitel kell a vásárláshoz: " + amount;

    var listOfferDiv = document.createElement("div");
    listOfferDiv.setAttribute("id", "list-offer-div");
    
    modalBody.appendChild(amountToBorrow);
    ...
    function list() {
        ...
        showOffers("list-offer-div", amount);
    }

    setTimeout(list, 1);
}

function acceptOfferModal(modalHeader, modalBody, amount) {
...
*/
    ...
    let backToOffers = document.createElement("button");
    backToOffers.innerHTML = "Vissza";
    backToOffers.setAttribute("onclick", "backToOffersFx(" + amount + ")");
    backToOffers.setAttribute("class", "btn btn-primary mx-2");
    console.log("AcceptOfferModal:", amount);
    ...
    modalBody.appendChild(backToOffers);
}

function registerModal(modalHeader, modalBody, amount) {
    ...
    regSubmitButton.setAttribute("onclick", "regSubmit(" + amount + ")");
...
function regSubmit(amount) {
    ...
    myAjax("POST", "http://" + host + "/register", req, function(status, data) {
        ...
        if(data == "SUCCESS") {
           ...
           // $("#reg-user").empty();
            ...
            alert("Sikeres regisztráció, nem sokára értesítünk emailben a további teendőkről");
        }
...
function acceptOffer() {
    ...
    if(amount == 0 || amount == "") {
        ...
        myAjax("POST", "http://" + host + "/acceptOffer", req, (err, res) => {
            if(!err) {
                ...
                if(res == "SUCCESS") {
                    ...
                    alert("HIBA, ellenőrizd a hitel felvétel részleteit!");
                }                
            } else {
                console.log(err);
            }
...
function orderBy(id) {
    console.log(id);
    sortType = id;
    listOffersToTable(offerData, div);
}

function listOffersToTable(data, url, amount) {
    console.log("listOffersToTable:", amount);
    data.sort(function(a, b) {return b[sortType] - a[sortType]});
    var prevLength = document.getElementsByClassName("mc-offer-table-body").length;
    for(var c = prevLength - 1; c >= 0; c--) {
        document.getElementById(url).removeChild(document.getElementsByClassName("mc-offer-table-body")[c]);
    }
    for(var a = 0; a < data.length; a++) {
        if(data[a].amount > 1999 && data[a].amount >= amount) {
            var newUl = document.createElement("ul");
            newUl.innerHTML = "<li>" + data[a].amount + "MX</li><li>" + data[a].length / 86400 + "nap</li><li>" + data[a].periods + "</li><li>" + data[a].interest / 10 + "%</li><li>" + data[a].score + "</li>";
            document.getElementById(url).appendChild(newUl);
            //console.log("a:", a);
            currentOffer = a;
            newUl.setAttribute("class", "mc-offer-table-body");
            newUl.setAttribute("onmouseover", "style.background='black'");
            newUl.setAttribute("onmouseout", "style.background='gray'");
            newUl.setAttribute("onclick", "getOfferDetails(" + amount + ")");
            newUl.style.listStyleType = "none";
            newUl.style.padding = "3px";
            newUl.style.marginTop = "0";
            newUl.style.marginBottom = "0";

            setElementsStyle(newUl, "li", "display", "inline-block");
            setElementsStyle(newUl, "li", "color", "white");
            setElementsStyle(newUl, "li", "margin", "2px");
            setElementsStyle(newUl, "li", "padding", "4px");
            setElementsStyle(newUl, "li", "width", "120px");
            setElementsStyle(newUl, "li", "text-align", "right");
        }
    }
}

function showOffers(url, amount) {
    //console.log("show", url);
    console.log("showOffers:", amount);
    div = url;
    document.getElementById(url).style.background = "gray";
    //document.getElementById(url).style.height = "100%";
    document.getElementById(url).style.width = "700px";
    
    var ul = document.createElement("ul");
    
    ul.innerHTML = "<li onclick=orderBy(\"amount\")>Összeg</li><li onclick=orderBy(\"length\")>Hossz</li><li onclick=orderBy(\"periods\")>Periódusok</li><li onclick=orderBy(\"interest\")>Kamat</li><li onclick=orderBy(\"score\")>Min. minősítés</li>";
    //ul.innerHTML = "<li name=amount class=mc-select-active>Összeg</li><li class=mc-select name=length>Hossz</li><li class=mc-select name=periods>Periódusok</li><li class=mc-select name=interest>Kamat</li><li class=mc-select name=score>Min. minősítés</li>";
    ul.setAttribute("id", "mc-offer-table-head");
    document.getElementById(url).appendChild(ul);
    setElementsStyle(ul, "li", "display", "inline-block");
    setElementsStyle(ul, "li", "color", "white");
    setElementsStyle(ul, "li", "margin", "2px");
    setElementsStyle(ul, "li", "padding", "4px");
    setElementsStyle(ul, "li", "width", "120px");
    ul.style.listStyleType = "none";
    ul.style.padding = "3px";
    ul.style.marginTop = "0";
    ul.style.marginBottom = "0";
    ul.style.borderBottom = "1px white solid";

    myAjax("POST", "http://" + host + "/offers", "", function(err, res) {
        if(!err) {
            //console.log(res);
            let data = JSON.parse(res);
            offerData = data;
            listOffersToTable(data, url, amount);
        } else {
            console.log(err);
        }
    });
...
function getOfferDetails(amount) {
    closeModal();
    createModal("acceptOffer", amount);
}

function backToOffersFx(amount) {
    closeModal();
    createModal("listOffers", amount);
}
